...
var map_size = Global.SMALL_SIZE
var translations = []
...
func generate():
	...
	_pivot.scale = Vector3(map_size.size, 1, 1)
...
func _update_each_spawn_translations():
	...
	_spawn_translations[0].translation += Vector3(multiplier * map_size.size,0,0) # -multiplier * map_size.size)
	_spawn_translations[1].translation += Vector3(multiplier * map_size.size,0,0)
	_spawn_translations[2].translation += Vector3(multiplier * map_size.size,0,0) # multiplier * map_size.size)
	
	_spawn_translations[3].translation -= Vector3(multiplier * map_size.size,0,0) # multiplier * map_size.size)
	_spawn_translations[4].translation -= Vector3(multiplier * map_size.size,0,0)
	_spawn_translations[5].translation -= Vector3(multiplier * map_size.size,0,0) # -multiplier * map_size.size)
...
func _create_box_shape_translations() -> Array:
	...
	var unit_pos = Vector3(waypoint_position.x, 0 ,waypoint_position.z) - space_between_units * Vector3(square_side_size/2 * map_size.size,0,square_side_size/2 / map_size.size)
	for i in number_of_unit:
		pos.append(unit_pos  + Vector3(0,0.5,12))
		unit_pos.x += space_between_units
		if unit_pos.x / map_size.size > (waypoint_position.x + square_side_size * space_between_units / 2):
			unit_pos.z += space_between_units
			unit_pos.x = (waypoint_position.x - space_between_units * square_side_size / 2) * map_size.size
	return pos
